"""HMAC-SHA256 request signing for the BloodHound API.

BloodHound Enterprise and Community Edition share the same authentication
scheme, so this is lifted verbatim from the proven Pathstrike implementation.
"""
...
class HMACAuth:
    """Generates HMAC-SHA256 signed headers for BloodHound API requests.

    BloodHound uses a three-layer HMAC construction:
    ...
    by BloodHound.  It is used directly as UTF-8 bytes for the HMAC key (it is
    **not** base64-decoded first - this matches the official BloodHound SDK).
    """

    def __init__(self, token_id: str, token_key: str) -> None:
        """Initialize with BloodHound API token credentials.
